...
void propensity_update(double** propensity, double* propensity_sums, int cell_idx, int* wildtype_populations, int* ra_populations, double degradation_rate, double diffusion_rate, double nucleus_control_factor, double target_population, double rate_difference) {
	...
	double ra_replication_rate = wildtype_replication_rate - rate_difference;
	
	propensity[cell_idx][0] = degradation_rate * cell_wildtype_population; //mort de wildtypes 
	propensity[cell_idx][1] = fmax(0.0, wildtype_replication_rate * cell_wildtype_population);//naissance de wildtypes
	propensity[cell_idx][2] = 2 * diffusion_rate * cell_wildtype_population; //diffusions de wildtypes 
	propensity[cell_idx][3] = (degradation_rate - rate_difference) * cell_ra_population; //mort slowest mutants 
	propensity[cell_idx][4] = fmax(0.0, ra_replication_rate * cell_ra_population); //naissance slowest mutants 
...
int main(int argc, char *argv[]) {
	...
	const double rate_difference = RATE_DIFFERENCE;
	...
	printf("RNG initialized with seed: %d\n", seed);
	/* end of GSL setup */
	...
	fprintf(fp_parameters, "rate_difference,%e\n", RATE_DIFFERENCE);
	...
	sprintf(ra_population_filename, "sss_sim_populations%d.txt", seed);
	...
	sprintf(ra_sfs_filename, "sss_sim_site_frequency_spectrum%d.txt", seed);
	...
	for (int k=0; k<CELLS; ++k) {
		...
		propensity_update(propensity, propensity_sums, k, wildtype_populations, ra_populations, degradation_rate, diffusion_rate, nucleus_control_factor, target_population, rate_difference);
	}
	...
	while (current_time<SIM_LENGTH && n_event<MAX_N_EVENTS) {
		...
		gillespie_event(rng, propensity, propensity_sums, wildtype_state, ra_state, mutant_counts, wildtype_populations, ra_populations, site_std_mutation_rate, degradation_rate, diffusion_rate, nucleus_control_factor, target_population, rate_difference);
		n_event++;
