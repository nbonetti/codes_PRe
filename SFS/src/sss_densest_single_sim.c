...
void propensity_update_2(double** propensity, double* propensity_sums, int cell_idx, int* wildtype_populations, int* ssd_populations, double degradation_rate, double diffusion_rate, double nucleus_control_factor, double target_population, double density, double rate_difference) {
	...
	propensity[cell_idx][3] = (degradation_rate - rate_difference) * cell_ssd_population;
	propensity[cell_idx][4] = (replication_rate-rate_difference) * cell_ssd_population;
...
int main(int argc, char *argv[]) {
    ...
    fprintf(fp_parameters, "rate_difference,%d\n", RATE_DIFFERENCE);
	...
	sprintf(ssd_population_filename, "sss_densest_sim_populations%d.txt", seed);
	...
	sprintf(ssd_sfs_filename, "sss_densest_sim_site_frequency_spectrum%d.txt", seed);
	...
	for (int k=0; k<CELLS; ++k) {
		...
		propensity_update_2(propensity, propensity_sums, k, wildtype_populations, ssd_populations, degradation_rate, diffusion_rate, nucleus_control_factor, target_population, density, rate_difference);
	}
	...
	while (current_time<SIM_LENGTH && n_event<MAX_N_EVENTS) {
		...
		gillespie_event_2(rng, propensity, propensity_sums, wildtype_state, ssd_state, mutant_counts, wildtype_populations, ssd_populations, site_std_mutation_rate, degradation_rate, diffusion_rate, nucleus_control_factor, target_population, density, rate_difference);
		n_event++;
